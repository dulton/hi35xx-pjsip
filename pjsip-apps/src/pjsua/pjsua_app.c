...
//extern static char lance_call_start;



static HI_S32 audio_destroy(void)
{
	//HI_S32 i, s32Ret;
	...
	********************************************
    ...
    for (i=0; i<audio_config->s32AencChnCnt; i++)
    {
        audio_config->AeChn = i;
        audio_config->AiChn = i;
		audio_config->AoChn = i;
		audio_config->AdChn = i;


		if (HI_TRUE == audio_config->bSendAdec)
		{
	        SAMPLE_COMM_AUDIO_DestoryTrdAencAdec(audio_config->AdChn);
	        SAMPLE_COMM_AUDIO_StopAo(audio_config->AoDev, audio_config->AoChn, audio_config->gs_bAioReSample);
	        SAMPLE_COMM_AUDIO_StopAdec(audio_config->AdChn);
	        SAMPLE_COMM_AUDIO_AoUnbindAdec(audio_config->AoDev, audio_config->AoChn, audio_config->AdChn);
	    }

			
        if (HI_TRUE == audio_config->gs_bUserGetMode)
        {
            SAMPLE_COMM_AUDIO_DestoryTrdAi(audio_config->AiDev, audio_config->AiChn);
        }
        else
        {        
            SAMPLE_COMM_AUDIO_AencUnbindAi(audio_config->AiDev, audio_config->AiChn, audio_config->AeChn);
        }

    }
    
    SAMPLE_COMM_AUDIO_StopAenc(audio_config->s32AencChnCnt);
    SAMPLE_COMM_AUDIO_StopAi(audio_config->AiDev, audio_config->s32AiChnCnt, 
				audio_config->gs_bAiAnr, audio_config->gs_bAioReSample);
	*/
...
static void ring_stop(pjsua_call_id call_id)
{
    ...
    if (app_config.call_data[call_id].ringback_on) 		//回铃声
	{
		app_config.call_data[call_id].ringback_on = PJ_FALSE;			//清标志

		pj_assert(app_config.ringback_cnt>0);								//断言数目多于0个
		if (--app_config.ringback_cnt == 0 && 
		    app_config.ringback_slot!=PJSUA_INVALID_ID) 
		{
		    pjsua_conf_disconnect(app_config.ringback_slot, 0);		//断开源到目的端口的媒体流
		    pjmedia_tonegen_rewind(app_config.ringback_port);			//跳到播放列表头回放
		}
    }

    if (app_config.call_data[call_id].ring_on) 			//按铃
	{
		...
		{
		    pjsua_conf_disconnect(app_config.ring_slot, 0);				//断开源到目的端口的媒体流
		    pjmedia_tonegen_rewind(app_config.ring_port);				//跳到播放列表头回放
		}
    }
}
...
static void ring_start(pjsua_call_id call_id)
{
    if (app_config.no_tones)							//静音模式
		return;

    if (app_config.call_data[call_id].ring_on)	//已设置
		return;
	...
	app_config.ring_slot!=PJSUA_INVALID_ID) 
    {
		pjsua_conf_connect(app_config.ring_slot, 0);	//输入响铃声音?
    }
}
...
/*
 * Handler when invite state has changed.		邀请状态改变时，
 */
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
    ...
    pjsua_call_get_info(call_id, &call_info);				//获取call_id的细节信息

    if (call_info.state == PJSIP_INV_STATE_DISCONNECTED) {

	/* Stop all ringback for this call */					//停止所有二次响铃
	ring_stop(call_id);	
	...
	/* Rewind play file when hangup automatically, 		当挂起的时候自动回放文件
	...
	PJ_LOG(3,(THIS_FILE, "Call %d is DISCONNECTED [reason=%d (%s)]", 
		  ...
		  call_info.last_status_text.ptr));			//打印出断开的ID号 

	if (call_id == current_call) 						//当前会话挂断或按了"]"时  调用
	{
	    find_next_call();
	}
	...
	if (app_config.duration != PJSUA_APP_NO_LIMIT_DURATION && 
	    call_info.state == PJSIP_INV_STATE_CONFIRMED) 				//状态确认?
	{
	    /* Schedule timer to hangup call after the specified duration */		//在一定周期后 安排定时器挂断呼叫
	    app_call_data *cd = &app_config.call_data[call_id];
	    pjsip_endpoint *endpt = pjsua_get_pjsip_endpt();
	    pj_time_val delay;
		
	    cd->timer.id = call_id;
	    delay.sec = app_config.duration;						//持续时间
	    delay.msec = 0;
	...
	if (call_info.state == PJSIP_INV_STATE_EARLY) {
	    ...
	    /* This can only occur because of TX or RX message */		//接收、发送信息时才发生
...
//null-audio---->08:08:23.930	pjsua_app.c  .......Call 0 state changed to CALLING
//null-audio---->08:08:29.785  pjsua_app.c  .....Call 0 state changed to CONNECTING
...
/**
 * Handler when there is incoming call.		//来电回调函数 
 */
			     ...
			     pjsip_rx_data *rdata)
{
    ...
    pjsua_call_get_info(call_id, &call_info);		//获取呼叫信息
    ...
    /* Start ringback */
    ring_start(call_id);			//启动二次响铃
    
    if (app_config.auto_answer > 0) 
	{
		pjsua_call_setting opt;

		pjsua_call_setting_default(&opt);			//初始化呼叫设置
...
#if PJSUA_HAS_VIDEO
		if (call_info.rem_offerer && call_info.rem_vid_cnt) 		//有视频 sdp 视频数
		{
...
//null-audio---->08:08:29.787	pjsua_app.c  .....Call 0 media 0 [type=audio], status is Active
    PJ_LOG(4,(THIS_FILE, "Call %d media %d [type=%s], status is %s",
	      ...
	      status_name[ci->media[mi].status]));		//打印出媒体ID号 索引 类型 状态 
}
                                ...
                                pj_bool_t *has_error)
{
    ...
    /* Stop ringback */
    ring_stop(ci->id);				//结束响铃

    /* Connect ports appropriately when media status is ACTIVE or REMOTE HOLD,
     * otherwise we should NOT connect the ports.			媒体状态是ACTIVE或REMOTE HOLD时，连接端口
     */
	...
	call_conf_slot = ci->media[mi].stream.aud.conf_slot;			//呼叫的回话端口号

	/* Loopback sound, if desired */
	if (app_config.auto_loop) {														//本地环回
	    pjsua_conf_connect(call_conf_slot, call_conf_slot);							//相同ID号
	    connect_sound = PJ_FALSE;
	}

	/* Automatically record conversation, if desired */
	if (app_config.auto_rec && app_config.rec_port != PJSUA_INVALID_ID) {		//自动会话录音
	...
	if ((app_config.auto_play || app_config.auto_play_hangup) && 
	    app_config.wav_port != PJSUA_INVALID_ID)										//保存为文件
	{
	...
	if (app_config.avi_auto_play &&
	    ...
	    app_config.avi[app_config.avi_def_idx].slot != PJSUA_INVALID_ID)		//保存为AVI格式
	{
	...
	if (app_config.auto_conf) {														//将呼叫加入群体会话(群聊模式?)
	    pjsua_call_id call_ids[PJSUA_MAX_CALLS];
	    unsigned call_cnt=PJ_ARRAY_SIZE(call_ids);
	    unsigned i;

	    /* Get all calls, and establish media connection between				获取所有呼叫，在这呼叫跟其它者之间建立连接
		...
		{
			if (call_ids[i] == ci->id)						//呼叫ID冲突
			    continue;
			
			if (!pjsua_call_has_media(call_ids[i]))	//呼叫	有活跃的媒体会话==1  
			    continue;		

			pjsua_conf_connect(call_conf_slot,
					   pjsua_call_get_conf_port(call_ids[i]));		//返回会话端口ID
			pjsua_conf_connect(pjsua_call_get_conf_port(call_ids[i]),
			                   call_conf_slot);

			/* Automatically record conversation, if desired */		//自动会话录音
	    ...
	    /* Also connect call to local sound device */					//也将呼叫连接到本地音频设备
	    connect_sound = PJ_TRUE;
	}

	/* Otherwise connect to sound device */									//默认将呼叫连接到本地音频设备
	if (connect_sound) 
	{
	    pjsua_conf_connect(call_conf_slot, 0);
	    if (!disconnect_mic)
			pjsua_conf_connect(0, call_conf_slot);			//静音---0输入
                                ...
                                pj_bool_t *has_error)
{
    ...
    arrange_window(ci->media[mi].stream.vid.win_in);		//重排窗口
...
static void on_call_media_state(pjsua_call_id call_id)
{
	...
	{
		on_call_generic_media_state(&call_info, mi, &has_error);		//获取媒体ID号 索引 类型 状态 

		switch (call_info.media[mi].type) 			//媒体类型
		{
			case PJMEDIA_TYPE_AUDIO:						//处理音频状态---环回、自动会话录音、保存为文件、保存为AVI、加入群聊
			    on_call_audio_state(&call_info, mi, &has_error);			
			    break;
			case PJMEDIA_TYPE_VIDEO:						//处理视频状态---
    ...
    if (call_info.rem_offerer && call_info.rem_vid_cnt)				//SDP及视频流
    {
		int vid_idx;

		/* Check if there is active video */
		vid_idx = pjsua_call_get_vid_stream_idx(call_id);		//获取视频
...
/*
 * Notification on sound device operation.		通知音频设备操作
 */
static pj_status_t on_snd_dev_operation(int operation)
{

//null-audio---->08:07:53.808    pjsua_app.c  ...Turning sound device ON
...
{
    /* Notify app */
    if (app_cfg.on_started) 		//是否启动?
	{
		if (status == PJ_SUCCESS) 
		{
		    char info[128];
		    cli_get_info(info, sizeof(info));		//客户端获取信息
		    if (app_cfg.on_started) 
			{
				(*app_cfg.on_started)(status, info);		//再次确认	
		    } 
		} 
		else 
		{
...
//初始化客户端&
    ...
    /** Parse args **/
    status = load_config(app_cfg.argc, app_cfg.argv, &uri_arg);		//加载配置
    if (status != PJ_SUCCESS) 
	{
    ...
    /* Initialize application callbacks */		//初始化应用的回调函数
    app_config.cfg.cb.on_call_state = &on_call_state;
    app_config.cfg.cb.on_call_media_state = &on_call_media_state;			//处理媒体状态
    app_config.cfg.cb.on_incoming_call = &on_incoming_call;					//来电
    ...
    /* Set sound device latency */			//延迟发送音频设备
    ...
    /* Initialize our module to handle otherwise unhandled request */		//初始化请求处理模块
    ...
    /* Initialize calls data */			//初始化呼叫数据
    for (i=0; i<PJ_ARRAY_SIZE(app_config.call_data); ++i) 
	{
		app_config.call_data[i].timer.id = PJSUA_INVALID_ID;				//通过ID区分相同回调函数及用户数据的多个定时器记录
		app_config.call_data[i].timer.cb = &call_timeout_callback;	//最大呼叫持续时间
    }

    /* Optionally registers WAV file */  			//按需注册音频文件
    for (i=0; i<app_config.wav_count; ++i) 
	{
		...
		status = pjsua_player_create(&app_config.wav_files[i], play_options, &wav_id);		//创建播放文件
		if (status != PJ_SUCCESS)
	    	goto on_error;

		if (app_config.wav_id == PJSUA_INVALID_ID) 		//文件存在
		{
		    app_config.wav_id = wav_id;						//将获取的ID填充
		    app_config.wav_port = pjsua_player_get_conf_port(app_config.wav_id);
		    if (app_config.auto_play_hangup) 				//定义自动挂断
			{
				pjmedia_port *port;
    ...
    /* Optionally registers tone players */			//音调播放器
    ...
    /* Optionally create recorder file, if any. */			//按需 创建录音文件
    if (app_config.rec_file.slen) 
	{
		...
		app_config.rec_port = pjsua_recorder_get_conf_port(app_config.rec_id);		//录音接口
    }

    pj_memcpy(&tcp_cfg, &app_config.udp_cfg, sizeof(tcp_cfg));

    /* Create ringback tones */						//创建回铃声
    if (app_config.no_tones == PJ_FALSE) 
	{
		...
		samples_per_frame = app_config.media_cfg.audio_frame_ptime * 
				    ...
				    app_config.media_cfg.channel_count / 1000;		// 20ms * 8000 * 1 /1000 = 160
...
#else
		PJ_LOG(2,(THIS_FILE,"Warning: --play-avi is ignored because AVI is disabled"));
#endif	/* PJMEDIA_VIDEO_DEV_HAS_AVI */
    }
//////////end avi_cnt

    /* Add UDP transport unless it's disabled. */		//添加UDP传输口------直接使用!!!
    if (!app_config.no_udp) 
	{
		...
		/* Add local account */			//添加本地账户
		pjsua_acc_add_local(transport_id, PJ_TRUE, &aid);

		/* Adjust local account config based on pjsua app config */		//根据pjsua应用配置调整本地账户配置
		{
...
////////////////////////////////////////////////////////////////////////////////
    /* Add UDP IPv6 transport unless it's disabled. */		//添加IPv6传输口
    ...
    /* Add TCP transport unless it's disabled */			//添加TCP传输口
    ...
    /* Add TCP IPv6 transport unless it's disabled. */		//添加TCP IPv6传输口
...
#if defined(PJSIP_HAS_TLS_TRANSPORT) && PJSIP_HAS_TLS_TRANSPORT!=0
    /* Add TLS transport when application wants one */		//添加TLS传输口
    if (app_config.use_tls) 
	{

		pjsua_acc_id acc_id;

		/* Copy the QoS settings */				//复制QoS设置
    ...
    /* Add TLS IPv6 transport unless it's disabled. */		//添加TLS IPv6传输口
...
/**************************************************************************************/
    ...
    if (transport_id == -1) 		//本地账户 transport_id?
	{
    ...
    for (i=0; i<app_config.acc_cnt; ++i) 
	{
		...
		app_config_init_video(&app_config.acc_cfg[i]);					//初始化视频配置

		status = pjsua_acc_add(&app_config.acc_cfg[i], PJ_TRUE, NULL);	//添加账户
		if (status != PJ_SUCCESS)
		    goto on_error;
		pjsua_acc_set_online_status(current_acc, PJ_TRUE);						//设置在线状态
    }

    /* Add buddies */									//添加好友
    for (i=0; i<app_config.buddy_cnt; ++i) 
	{
		status = pjsua_buddy_add(&app_config.buddy_cfg[i], NULL);			//添加好友
		if (status != PJ_SUCCESS) 
		{
    ...
    /* Optionally disable some codec */			//视情况禁用一些编码
    ...
    if (app_config.capture_dev  != PJSUA_INVALID_ID || app_config.playback_dev != PJSUA_INVALID_ID) 		//无采集设备也无播放设备
    {
...
pj_status_t pjsua_app_init(const pjsua_app_cfg_t *cfg)
{
    pj_status_t status;
	HI_S32 s32Ret;
    pj_memcpy(&app_cfg, cfg, sizeof(app_cfg));		//相同类型的不同指针

    status = app_init();								//应用的初始化
    if (status != PJ_SUCCESS)
		return status;

    /* Init CLI if configured */    				//若配置完，初始化客户端
...
pj_status_t pjsua_app_run(pj_bool_t wait_telnet_cli)
{
	...
	{
		char info[128];
		cli_get_info(info, sizeof(info));		//获取客户端信息
		if (app_cfg.on_started) 
		{
    ...
    /* If user specifies URI to call, then call the URI */		//如果用户呼叫指定URL，调用URI
    if (uri_arg.slen) 
	{
		pjsua_call_setting_default(&call_opt);								//初始化呼叫设置
		call_opt.aud_cnt = app_config.aud_cnt;
		call_opt.vid_cnt = app_config.vid.vid_cnt;

		/*使用指定用户去呼叫指定URI*/
		pjsua_call_make_call(current_acc, &uri_arg, &call_opt, NULL,NULL, NULL);	//打开设备(海思无设备..)
    }   
	...
	/***************************************************
	 *			有客户时----接入后直接建立通信
	 *			此处 wait_telnet_cli 为 true
	 *
	****************************************************/
    if (app_config.use_cli)			
    {
		status = cli_main(wait_telnet_cli);		//打开日志 等待退出....
	}
    else
		legacy_main();					//主函数用户界面环回
