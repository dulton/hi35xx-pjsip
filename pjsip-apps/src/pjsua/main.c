...
int main_func(int argc, char *argv[])
{
    ...
    pj_bzero(&cfg, sizeof(cfg));				//清零、填充参数
    cfg.on_started = &on_app_started;		//客户端启动(重启)时调用
    ...
    while (running) 
	{        
		status = pjsua_app_init(&cfg);				//初始化
		if (status == PJ_SUCCESS) 
		{
	    	status = pjsua_app_run(PJ_TRUE);			//运行
